(function(global){
  ...
  function installStyle(){
    ...
    style.textContent=`
      ...
      @media(max-width:380px){.rent-detail-summary-v{font-size:14px}.rent-detail-room{grid-template-columns:minmax(0,1fr) 74px}.rent-detail-paidamt{min-width:68px}.rent-detail-contract{font-size:9px}}
    `;
  ...
  function sortRooms(rooms){
    return [...rooms].sort((a,b)=>String(a?.roomName||'').localeCompare(String(b?.roomName||''),'ko',{numeric:true,sensitivity:'base'}));
  }
  ...
  function installRentDetail(options,apiLoad){
    ...
    function ensureSheet(){
      ...
      root.hidden=true;
      root.innerHTML=`<section id="rentDetailSheet" role="dialog" aria-modal="true" aria-labelledby="rentDetailTitle"><div class="rent-detail-handle"></div><div class="rent-detail-head"><div><div id="rentDetailTitle" class="rent-detail-title">임대 현황</div><div id="rentDetailAddress" class="rent-detail-address"></div></div><button type="button" class="rent-detail-close" aria-label="닫기">×</button></div><div class="rent-detail-monthbar"><button type="button" class="rent-detail-monthbtn" data-rent-month="-1" aria-label="이전 달">‹</button><div id="rentDetailMonth" class="rent-detail-month"></div><button type="button" class="rent-detail-monthbtn" data-rent-month="1" aria-label="다음 달">›</button></div><div id="rentDetailBody"></div></section>`;
    ...
    function renderProperty(property){
      ...
      const rooms=sortRooms(Array.isArray(property?.rooms)?property.rooms:[]);
