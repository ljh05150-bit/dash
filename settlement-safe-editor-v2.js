(function(){
  ...
  function injectStyle(){
    ...
    s.textContent=`
      .safe-v2-back[hidden]{display:none!important}
      ...
      .safe-v2-handle{width:42px;height:5px;border-radius:999px;background:#40536a;margin:2px auto 15px}
      .safe-v2-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;padding-bottom:14px;border-bottom:1px solid var(--line2)}
      .safe-v2-title{font-size:20px;font-weight:850;line-height:1.3}.safe-v2-sub{font-size:11px;color:var(--muted);margin-top:5px;line-height:1.5}.safe-v2-amount{font-size:18px;font-weight:850;margin-top:5px;color:#8abfff}
      .safe-v2-close{width:38px;height:38px;border:1px solid #294c6d;background:#102137;color:#dbe8f5;border-radius:13px;font-size:21px;flex:0 0 auto}
      .safe-v2-field{margin-top:14px}.safe-v2-field label{display:block;font-size:11px;color:var(--muted);margin-bottom:6px}
      .safe-v2-field select,.safe-v2-field input[type="text"]{width:100%;border:1px solid #2a4d6d;background:#081525;color:var(--text);border-radius:13px;padding:12px;font-size:13px;outline:none}
      .safe-v2-field select:focus,.safe-v2-field input[type="text"]:focus{border-color:#4b9dff;box-shadow:0 0 0 2px rgba(75,157,255,.13)}
  ...
  function ensureModal(){
    ...
    root.innerHTML=`<section class="safe-v2-sheet" role="dialog" aria-modal="true" aria-labelledby="safeV2Title">
      ...
      <div class="safe-v2-field"><label for="safeV2Category">구분</label><select id="safeV2Category"></select></div>
    ...
    root.addEventListener('click',event=>{
      if(event.target===root||event.target.closest('.safe-v2-close')||event.target.closest('[data-v2-cancel]'))closeModal();
  ...
  async function openById(id){
    ...
    try{
      ...
      root.dataset.originalCategory=data.category||'미분류';
      root.querySelector('#safeV2Title').textContent=data.merchant||data.category||'거래';
      ...
      root.querySelector('#safeV2Category').innerHTML=optionMarkup(data.category||'미분류');
      ...
      root.querySelector('#safeV2Category').focus({preventScroll:true});
    }catch(error){
  ...
  async function saveCurrent(){
    ...
    const category=root.querySelector('#safeV2Category').value||'미분류';
  ...
  function setup(){
    ...
    document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!ensureModal().hidden)closeModal();});
  }
