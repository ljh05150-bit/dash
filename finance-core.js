...
(function(global){
  ...
  function forceOpenModal(tx){
    ...
    root.addEventListener('click',e=>{
      ...
      if(e.target.closest('[data-recent-save]')){
        e.preventDefault();
        if(typeof global.saveRecentTransaction==='function')global.saveRecentTransaction(String(tx.id));
      }
    });
  ...
  function installStableRecentRenderer(){
    ...
    if(typeof global.renderTransactions!=='function'){setTimeout(installStableRecentRenderer,0);return}
    ...
    global.renderTransactions=function(tx){
      ...
      host.innerHTML=visible.map(t=>{
        ...
        return `<div class="tx-row" role="button" tabindex="0" data-tx-id="${esc(id)}"><div class="tx-time">${esc(when)}</div><div class="tx-main"><div class="tx-merchant">${esc(t.merchant||t.category||'거래')}</div><div class="tx-account">${esc(t.account||t.source||'')} · ${esc(t.category||'미분류')}</div></div><div class="tx-amt neg">-₩${Math.round(a).toLocaleString('ko-KR')}</div></div>`;
  ...
  function keepServiceWorkerFresh(){
    ...
    global.addEventListener('load',()=>{
      setTimeout(()=>navigator.serviceWorker.register('./service-worker.js?rev=20260913-1315',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{}),500);
    },{once:true});
  }
